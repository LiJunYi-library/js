import { addEventListenerOnce, removeEventListener } from "../utils";
...
export function eventDecideToucth(props = {}) {
  const fn = () => () => undefined;
  const config = {
    maxOrientation: 10,
    COMPUTE_INTERVAL: 30,
...
export function eventDecideToucthOrientation() {
  function start(event) {}
  ...
  function addEvents(view) {
    if (window.PointerEvent) {
      addEventListenerOnce(view, "pointerdown", start);
      addEventListenerOnce(view, "pointermove", move);
      addEventListenerOnce(view, "pointerup", end);
    } else {
      addEventListenerOnce(view, "touchstart", start);
      addEventListenerOnce(view, "touchmove", move);
      addEventListenerOnce(view, "touchend", end);
    }
  }

  function removeEvents(view) {
    if (window.PointerEvent) {
      removeEventListener(view, "pointerdown", start);
      removeEventListener(view, "pointermove", move);
      removeEventListener(view, "pointerup", end);
    } else {
      removeEventListener(view, "touchstart", start);
      removeEventListener(view, "touchmove", move);
      removeEventListener(view, "touchend", end);
    }
  }
}
