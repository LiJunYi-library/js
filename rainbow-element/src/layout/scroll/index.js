...
export class RScroll extends RainbowElement {
  ...
  append(...nodes) {
    this.$slotContainer.default?.append?.(...nodes);
    console.log("append")
    for (const key in this.$slotContainer) {
      if (Object.prototype.hasOwnProperty.call(this.$slotContainer, key)) {
        if (key !== "default") {
          const slotContainer = this.$slotContainer[key];
          const slot = this.$.getSolts(this.$slotContainer.default)[key];
          if (slot?.children) slotContainer.append(...slot.children);
        }
      }
    }
  }
  ...
  appendChild(node) {
    this.$slotContainer.default?.appendChild?.(node);
  ...
  insertBefore(node, child) {
    this.$slotContainer.default.insertBefore?.(node, child);
  ...
  connectedCallback(...arg) {
    ...
    this.$$.refresh.render = this.$$renderRefresh();
    this.$$.prveScrollTop = this.$$.default.scrollTop;
    console.log(this.$.getSolts(this.$$.default));
  }
}
