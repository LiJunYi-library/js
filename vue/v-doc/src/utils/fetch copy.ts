...
const opt: MYFetchConfig = {
  ...
  interceptResponseSuccess(res, data, config) {
    if (data?.Code !== 0) {
      RGlobalToast.show({ text: data.Message });
...
interface SelectItem<V, L> {
  value: V;
  label: L;
}

type ASD<T, V, L> = SelectItem<V, L> & T;

class RA<V, L, T extends Record<string, V | L>> {
  select?: T;

  formatterCount = (item: T) => item.label;
}
