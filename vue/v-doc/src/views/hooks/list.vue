<template>
  <div class="hooks-radio">
    <div>
      ...
      <button @click="multipleList.add(creates([102, 103]))">add</button>
      <button @click="multipleList.unshift(...creates([104, 105]))">unshift</button>
      <button @click="multipleList.increase(creates([106, 107]))">increase</button>
      <button @click="multipleList.remove(multipleList.list[6], multipleList.list[3])">remove</button>
      <button @click="multipleList.expurgate(multipleList.select)">expurgate</button>
    ...
    <ElPagination layout="total, sizes, prev, pager, next, jumper" v-model:current-page="multipleList.currentPage"
      v-model:page-size="multipleList.pageSize" :total="multipleList.total" @change="changePagin">
    </ElPagination>
  </div>
</template>
<script setup>
