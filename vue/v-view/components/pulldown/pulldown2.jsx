...
export const RPulldown2 = defineComponent({
  props: {
  ...
  setup(props, context) {
    ...
    return () => {
      return (
        <div
          class={createClass("r-pulldown")}
          ref={(el) => (pulldownHtml.value = el)}
          onTouchstart={onTouchstart}
          onClick={onClick}
          style={{ zIndex: visible.value ? props.zIndex + 1 : props.zIndex }}
        >
          <div class={createClass("r-pulldown-content")}>
            {renderSlot(context.slots, "content", ctx, () => [
              <div class={createClass("r-pulldown-text")}>
                {renderSlot(context.slots, "label", ctx, () => [
                  <div class={createClass("r-pulldown-label")}> {props.activeLabel || props.label} </div>,
                ])}
                <span class={createClass("r-pulldown-icon")}>
